...
"""Script de validacion de datos OT a Aprobada del WF  preparaci\xf3n de Tareas
...
def titulo_atributo(atributo, objeto):
    ...
    try:
        ...
        logger.error("Error:Script de validacion de datos OT a Aprobada del WF  preparaci\xf3n de Tareas : titulo_atributo")
        logger.error(e)
try:
    userInfo = mbo.getUserInfo()
    campos=""
    ...
    if not lista.isEmpty():
        ...
        if faltaDatos:
            ...
            fechaHoy = MXServer.getMXServer().getDate()
            newWO.changeStatus("APROBADA", fechaHoy,"Aprobada por el WF")
            setOT.save()
            setOT.close()
    else:
        logger.error("Error: Script de validacion de datos OT a Aprobada del WF  preparaci\xf3n de Tareas")
        params=["No existe preparaci\xf3n de tareas"]
        errorkey = "UTD_GENERICException"
        errorgroup = "UTD_GENERICException"
        error = True 
except (MXException, MXApplicationException, RemoteException, Exception), e:
    logger.error("Error: Error: Script de validacion de datos OT a Aprobada del WF  preparaci\xf3n de Tareas")
    logger.error(e)
    params=["Ocurrio un error a la hora de procesar el ScriptError: Script de validacion de datos OT a Aprobada del WF  preparaci\xf3n de Tareas \n" + str(e.getMessage())]
    errorkey = "UTD_GENERICException"
    errorgroup = "UTD_GENERICException"
    error = True